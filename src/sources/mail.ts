...
export const extractMetricsFromEmailAttachments = (
  parsed: ParsedMail,
  ...
  if (!parsed.attachments || parsed.attachments.length === 0) {
    logger.info(`[${source.name}] No attachments found in email`);
    return metrics;
  }
  ...
  for (const attachment of parsed.attachments) {
    ...
    logger.info(`[${source.name}] Processing attachment: ${filename}`);
    ...
    try {
      ...
      const formattedDate = dayjs(extractedDate, source.dateFormat).format(
        "YYYY-MM-DD"
      );
      ...
      if (
        records.length > source.valueCell.row - startIndex &&
        records[source.valueCell.row - startIndex].length >= source.valueCell.column
      ) {
        const costCenter =
          records[source.costCenterCell.row - startIndex][
            source.costCenterCell.column - 1
          ];
        const value =
          records[source.valueCell.row - startIndex][source.valueCell.column - 1];

        if (costCenter && value) {
          const metricTypeMapping = source.metricTypeMappings.find(
            (m) => m.importName === "Umsatz"
          );

          const metricType = source.mergeMetricTypes.enabled
                  ...
                  msg.on("body", (stream) => {
                    ...
                    simpleParser(readableStream)
                      .then(async (parsed: ParsedMail) => {
                        metrics.push(
                          ...extractMetricsFromEmailAttachments(
                            parsed,
                            source,
                            timeZone
                          )
